...
foreach ($file in $files) {
    ...
    foreach ($name in $genericNames) {
        ...
        if ([regex]::IsMatch($content,$pattern)) {
            [void]$issues.Add(
                $file.FullName + ": unsafe Generic.List wrapper"
            )
        }
    }
...
if ($issues.Count -gt 0) {
    ...
    throw "v0.8.0 safety harness failed."
}
...
$runner = Join-Path $PackageRoot "tools\Run-Historical-Acquisition-v0.8.0.ps1"
...
if ($LASTEXITCODE -ne 0) {
    throw (
        "v0.8.0 noninteractive dry-run failed with exit code " +
        $LASTEXITCODE
    )
}

Write-Host "NonInteractive acquisition dry-run: PASS"
