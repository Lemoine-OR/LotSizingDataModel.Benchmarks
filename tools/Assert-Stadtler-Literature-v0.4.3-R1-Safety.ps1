...
foreach ($file in $files) {
    ...
    if ($content -match '\.Replace\s*\(\s*\[char\]') {
        [void]$issues.Add(
            $file.FullName +
            ": ambiguous Replace([char],...)"
        )
    }
}

$runner = Join-Path $PackageRoot `
    "tools\Run-Stadtler-Literature-v0.4.3-R1.ps1"

if (-not (Test-Path -LiteralPath $runner -PathType Leaf)) {
    [void]$issues.Add("Missing v0.4.3 R1 runner.")
}
else {
...
if ($issues.Count -gt 0) {
    ...
    throw "v0.4.3 R1 safety guard failed."
}
...
if ($LASTEXITCODE -ne 0) {
    throw (
        "v0.4.3 R1 dry-run failed with exit code " +
        $LASTEXITCODE
    )
}
