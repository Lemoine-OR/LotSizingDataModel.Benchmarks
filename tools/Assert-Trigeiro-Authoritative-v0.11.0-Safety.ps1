...
$runner = Join-Path $PackageRoot `
    "tools\Run-Trigeiro-Authoritative-v0.11.0.ps1"
...
$importerProgram = Join-Path $PackageRoot `
    "tools\TrigeiroAuthoritativeImporter\Program.cs"
...
if ($issues.Count -gt 0) {
    ...
    throw "v0.11.0 static safety harness failed."
}
...
Write-Host ""
Write-Host "==> Compile guard: authoritative Trigeiro importer"
...
if ($LASTEXITCODE -ne 0) {
    throw (
        "v0.11.0 C# compile guard failed with exit code " +
        $LASTEXITCODE
    )
}
...
Write-Host ""
Write-Host "==> NonInteractive authoritative dry-run"
...
if ($LASTEXITCODE -ne 0) {
    throw (
        "v0.11.0 dry-run failed with exit code " +
        $LASTEXITCODE
    )
}
