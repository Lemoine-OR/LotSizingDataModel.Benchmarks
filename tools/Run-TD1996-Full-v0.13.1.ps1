...
$checkerRoot = Join-Path $familyRoot "checker-reports\v0.13.1"
$reportRoot = Join-Path $BenchmarkRepo "reports\v0.13.1"
...
@(
    ...
    Export-Csv `
        -LiteralPath (Join-Path $metadataRoot "TD1996-PROVENANCE-v0.13.1.csv") `
        -NoTypeInformation `
        -Encoding UTF8
...
$inferredResourceAssignments = (
    $auditRows |
    Measure-Object -Property inferred_resource_assignments -Sum
).Sum
...
Write-Host ("All-zero CL source rows                      : " + $noResource)
Write-Host ("... with positive setup time                 : " + $positiveSetupNoResource)
Write-Host ("Resource assignments reconstructed           : " + $inferredResourceAssignments)
Write-Host ("Instances using documented Test Set C maps   : " + $documentedMapRows.Count)

if ($noResource -ne 1200 -or
    $inferredResourceAssignments -ne 1200 -or
    $documentedMapRows.Count -ne 1200) {
    throw "TD1996 semantic-repair postcondition failed: expected exactly 1200 documented item-29 resource reconstructions."
}
...
$cross.ToArray() |
    Export-Csv `
        -LiteralPath (Join-Path $metadataRoot "TD1996-CROSS-FAMILY-FINGERPRINTS-v0.13.1.csv") `
        -NoTypeInformation `
        -Encoding UTF8
...
$catalog.ToArray() |
    ...
    Export-Csv `
        -LiteralPath (Join-Path $metadataRoot "TD1996-CANONICAL-CATALOG-v0.13.1.csv") `
        -NoTypeInformation `
        -Encoding UTF8
...
$page.Add("")
$page.Add("> Complete user-provided 3450-instance corpus integrated in LotSizingDataModel.Benchmarks v0.13.1.")
$page.Add("")
...
$page.Add("")
$page.Add("Resource membership is explicit when a `CL_Produit` resource component is nonzero. The supplied format contains " + $noResource + " item rows whose entire setup-cost resource vector is zero. Of these, " + $positiveSetupNoResource + " have a positive scalar setup time. v0.13.1 does not invent a resource for those rows; the ambiguity is retained explicitly in the metadata catalogue.")
$page.Add("")
...
Write-Step "v0.13.1 final postconditions"
...
Write-Host "Structural Checker          : VALID"
Write-Host "Fingerprints                : 3450 / 3450"
Write-Host ("Zero CL source rows         : " + $noResource)
Write-Host ("Positive setup/zero CL      : " + $positiveSetupNoResource)
Write-Host ("Documented map reconstructions: " + $inferredResourceAssignments)
